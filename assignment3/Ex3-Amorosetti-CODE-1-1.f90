...
! Matrix multiplication Fortran code used in the last two previous assignements
! With small modifications, in order to execute it from a Python script and get the execution time

! The module implemented in the last assignements is not included this time
! Therefore the lines calling it are commented or even removed
...
program TestingPerformance
    ...
    ! We only use one variable time that we reuse each time we call a different method

    ! real(8) :: start_time, end_time
    ! real(8) :: time_omp 
    ! real(8) :: start_time_omp, end_time_omp

    integer :: start, end, rate ! Variables used to get the system clock rate, and measure the time with it (time = (end-start)/rate)
    

    character(len=10) :: arg1   ! Define a character variable to hold the command-line argument
    integer, allocatable :: A(:,:), B(:,:), C(:,:)

    character(len=256) :: method ! Variable used to store the multiplicaiton method asked by the Python script
    ...
    ! Commented as we modify the input of n to be given by the Python script
    ...
    call get_command_argument(1, method) ! Getting the input matrix multiplication method from the Python script 


    ! Get the command-line argument as a string, then convert it to an integer :

    call get_command_argument(2, arg1)  ! Getting the input matrix size from the Python script

    read(arg1, *) n   ! Convert the character argument to an integer
    ...
    ! Depending on the multiplication method given by the python script, different subroutines will be called : 

    if (trim(adjustl(method)) == "Standard") then ! Standard multiplication method

        call system_clock(count_rate=rate)        ! Getting the system clock rate, and measuring the clock ticks at the start and end 
        call system_clock(start)

        call matrix_multiplication_byrow(n, A, B, C)

        call system_clock(end)

        time = real(end - start) / rate
        print *, time

    elseif (trim(adjustl(method)) == "Column-wise") then ! Column-wise multiplication method

        call system_clock(count_rate=rate)
        call system_clock(start)

        call matrix_multiplication_bycolumn(n, A, B, C)
    ...
    elseif (trim(adjustl(method)) == "MATMUL()") then ! Built-in Fortran multiplication method
    ...
    elseif (trim(adjustl(method)) == "Standard, parallelized") then ! Standard, parallelized multiplication method

        call system_clock(count_rate=rate)
        call system_clock(start)

        call matrix_multiplication_byrow_parallel(n, A, B, C)
    ...
    elseif (trim(adjustl(method)) == "Column-wise, parallelized") then ! Column-wise, parallelized multiplication method

        call system_clock(count_rate=rate)
        call system_clock(start)

        call matrix_multiplication_bycolumn_parallel(n, A, B, C)
        ...
        print *, "Unknown methods" ! If the executable receive a wrong input for the multiplication method 
    end if


    ! In the previous assignements, commented here :
